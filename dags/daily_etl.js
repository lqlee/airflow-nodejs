...
export default dag({
    ...
    tasks: {
        ...
        transform: {
            ...
            run: async (ctx) => {
                const users = await ctx.xcom.pull('extract_users', 'users');
        ...
        load: {
            ...
            retries: 2,
            run: async (ctx) => {
                const enriched = await ctx.xcom.pull('transform', 'enriched');
        ...
        notify: {
            dependsOn: ['load'],
            run: async (ctx) => {
                const total = await ctx.xcom.pull('extract_users', 'count');
                const written = await ctx.xcom.pull('load', 'rows_written');

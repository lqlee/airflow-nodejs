...
export default dag({
    ...
    tasks: {
        ...
        check_nulls: {
            dependsOn: ['start'],
            run: async (ctx) => {
                const table = await ctx.xcom.pull('start', 'table');
        ...
        report: {
            ...
            run: async (ctx) => {
                const table = await ctx.xcom.pull('start', 'table');
                const startedAt = await ctx.xcom.pull('start', 'started_at');
                const nullResult = await ctx.xcom.pull('check_nulls', 'result');
                const dupeResult = await ctx.xcom.pull('check_dupes', 'result');
                const freshResult = await ctx.xcom.pull('check_freshness', 'result');

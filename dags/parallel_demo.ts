...
export default dag({
  ...
  tasks: {
    ...
    fetch_a: {
      dependsOn: ['start'],
      run: async (ctx) => {
        const config = await ctx.xcom.pull('start', 'config') as { batchSize: number }
    ...
    merge: {
      ...
      run: async (ctx) => {
        const rowsA = await ctx.xcom.pull('fetch_a', 'rows') as number
        const rowsB = await ctx.xcom.pull('fetch_b', 'rows') as number

...
export function analyzeWarnings(dag: DagDefinition, now = new Date()): DagWarning[] {
  ...
  for (const [taskId, task] of Object.entries(dag.tasks)) {
    // Task has no executable logic (shell, python, java, and container tasks count as run logic)
    if (!task.run && !task.poke && !task.shell && !task.python && !task.java && !task.container && !Array.isArray(task.expand)) {
      warn('no_run_logic',
